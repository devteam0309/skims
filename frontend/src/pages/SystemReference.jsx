...
const ROLE_GROUPS = [
  ...
  { name: 'FINANCE_EDITORS', roles: 'ADMINS + sk_treasurer', use: 'Create/edit budgets, expenses, liquidations. Replaced FINANCE_STAFF; sk_chairperson is now view-only in Fund Management.' },
  { name: 'STAFF', roles: 'ADMINS + sk_chairperson, sk_treasurer, sk_secretary, sk_kagawad, dilg_representative', use: 'Dashboard, monitoring, read access' },
  ...
  { name: 'FINANCE_APPROVERS', roles: 'ADMINS', use: 'Approve/reject expenses and liquidations. The treasurer records money and approves none of it; the chairperson does neither.' },
  { name: 'BUDGET_APPROVERS', roles: 'ADMINS', use: 'Approve/reject budgets' },
  { name: 'BARANGAY_BOUND_ROLES', roles: 'sk_chairperson, sk_treasurer, sk_secretary, sk_kagawad', use: 'Scope narrows to ONE barangay when the account names one. Admin tiers and DILG are never barangay-bound. utils/scope.js.' },
...
const API_ENDPOINTS = [
  ...
  },
  {
    ...
    routes: [
      ...
      { method: 'GET', path: '/email-changes', auth: 'protect', roles: 'super_admin', note: 'Accounts with an outstanding pendingEmail. Registered BEFORE /:id or the literal path is read as an id.' },
      ...
      { method: 'PUT', path: '/:id/email-change/approve', auth: 'protect', roles: 'super_admin', note: 'pendingEmail becomes email. Re-checks the address is still free. No re-verification: isEmailVerified is what the login gate reads, and on this deployment verification mail cannot reach an arbitrary inbox — requiring it would lock the owner out. AuditLog with old/new address.' },
      { method: 'PUT', path: '/:id/email-change/reject', auth: 'protect', roles: 'super_admin', note: 'Existing address stays active; reason stored and shown to the user; AuditLog + notification.' },
      { method: 'PUT', path: '/:id/barangay', auth: 'protect', roles: 'super_admin', note: 'Assigns or clears the barangay. Refuses one from outside the user own municipality. This is what makes barangay-level scope usable — nothing else ever set the field.' },
  ...
  },
  {
    ...
    routes: [
      ...
      { method: 'POST', path: '/', auth: 'protect', roles: 'FINANCE_EDITORS', note: 'budgetValidation; ALLOWED_CREATE_FIELDS; municipality forced from user' },
      { method: 'PUT', path: '/:id', auth: 'protect', roles: 'FINANCE_EDITORS', note: 'Pipeline update recalculates remainingBalance atomically; AuditLog' },
      ...
      { method: 'PATCH', path: '/:id/reopen', auth: 'protect', roles: 'FINANCE_EDITORS', note: 'approved/rejected→draft' },
  ...
  },
  {
    ...
    routes: [
      ...
      { method: 'GET', path: '/', auth: 'protect', roles: 'Any', note: 'Scoped; MAX_LIMIT=100' },
      { method: 'GET', path: '/:id', auth: 'protect', roles: 'Any', note: 'Municipality scope enforced' },
      { method: 'POST', path: '/', auth: 'protect', roles: 'FINANCE_EDITORS', note: 'expenseValidation; Cloudinary upload (max 10 attachments); budget status checked; linked programme must be in scope; saveAsDraft=true stores status draft; AuditLog' },
      { method: 'PUT', path: '/:id', auth: 'protect', roles: 'ADMINS + sk_treasurer', note: 'ALLOWED_UPDATE_FIELDS; municipality scope; AuditLog' },
      { method: 'PATCH', path: '/bulk-approve', auth: 'protect', roles: 'FINANCE_APPROVERS', note: 'Max 50; municipality- and barangay-scoped; self-approval blocked; same Budget/Program side effects as single approve' },
      { method: 'PATCH', path: '/:id/submit', auth: 'protect', roles: 'FINANCE_EDITORS', note: 'draft|rejected→pending, atomic; clears any rejection reason; AuditLog' },
      { method: 'PATCH', path: '/:id/reject', auth: 'protect', roles: 'FINANCE_APPROVERS', note: 'pending→rejected; rejectionReason REQUIRED; records rejectedBy/rejectedAt; AuditLog with old/new state; notification + email to the author' },
  ...
  },
  {
    ...
    routes: [
      { method: 'GET', path: '/', auth: 'protect', roles: 'Any', note: 'Scoped; MAX_LIMIT=100' },
      { method: 'GET', path: '/:id', auth: 'protect', roles: 'Any', note: 'Municipality scope enforced' },
      { method: 'POST', path: '/', auth: 'protect', roles: 'FINANCE_EDITORS', note: 'Cloudinary upload (max 20 documents); municipality forced from user; supportingDocuments[] links Document records, each checked against the caller scope; AuditLog' },
      { method: 'PATCH', path: '/:id/submit', auth: 'protect', roles: 'FINANCE_EDITORS', note: 'draft→submitted; AuditLog; email' },
  ...
  },
  {
    ...
    routes: [
      { method: 'GET', path: '/stats', auth: 'protect', roles: 'Any', note: 'Count by category + 5 most recent. NOW SCOPED — it previously had no role check and counted the whole province for any authenticated caller.' },
      { method: 'GET', path: '/recycle-bin', auth: 'protect', roles: 'ADMINS', note: 'Deleted documents (deletedAt set), scoped like the active list. Registered BEFORE /:id or "recycle-bin" is read as an id.' },
      ...
      { method: 'DELETE', path: '/:id', auth: 'protect', roles: 'ADMINS', note: 'Moves to the recycle bin. Records deletedBy and KEEPS the Cloudinary file — it used to destroy it in the same call, which made the surviving record unrestorable. AuditLog.' },
      { method: 'PATCH', path: '/:id/restore', auth: 'protect', roles: 'ADMINS', note: 'Clears deletedAt; the file was never destroyed so the document returns intact. AuditLog RESTORE.' },
      { method: 'DELETE', path: '/:id/permanent', auth: 'protect', roles: 'ADMINS', note: 'Irreversible. Only for a document already in the recycle bin. Destroys the file and every previousVersions file, then removes the record. Audited BEFORE deletion — afterwards the audit entry is the only trace.' },
    ],
  },
  {
    group: 'Youth — /api/youth',
    routes: [
      { method: 'POST', path: '/import/preview', auth: 'protect', roles: 'YOUTH_REGISTRARS', note: 'Parses an .xlsx and reports per-row validity, in-file duplicates and members already registered. Writes NOTHING.' },
      { method: 'POST', path: '/import', auth: 'protect', roles: 'YOUTH_REGISTRARS', note: 'Re-parses and re-validates the FILE (never rows echoed back by the client), inserts the valid rows one at a time, forces the barangay from the account, and writes one IMPORT AuditLog entry with the counts.' },
...
const PERMISSIONS_TABLE = [
  ...
  { module: 'Budgets — Create/Update', access: 'FINANCE_EDITORS (chairperson excluded)' },
  ...
  { module: 'Expenses — Create', access: 'FINANCE_EDITORS (chairperson excluded)' },
  { module: 'Expenses — Update', access: 'ADMINS + sk_treasurer' },
  { module: 'Expenses — Approve/Reject', access: 'FINANCE_APPROVERS = ADMINS (self-approval still blocked)' },
  { module: 'Expenses — Delete', access: 'ADMINS (blocked if approved/liquidated)' },
  { module: 'Liquidations — View', access: 'STAFF' },
  { module: 'Liquidations — Create/Submit', access: 'FINANCE_EDITORS (chairperson excluded)' },
...
const YOUTH_ALLOWLIST = [
  'GET/PUT /api/auth/me (email is NOT accepted here), PUT /api/auth/password, POST/DELETE /api/auth/me/email-change, POST /api/auth/logout',
